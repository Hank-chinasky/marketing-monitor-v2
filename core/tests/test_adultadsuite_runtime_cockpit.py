...
class AdultAdSuiteRuntimeCockpitTests(TestCase):
    ...
    def test_adultadsuite_cockpit_renders_revenue_day_plan_for_logged_in_user(self):
        ...
        self.assertContains(response, "Open in focusstand")
        self.assertContains(response, "Focusstand")
        self.assertContains(response, "focus=1")
        self.assertContains(response, "Normale stand: overzicht en live gesprekken")
        self.assertContains(response, "Focusstand: minder afleiding tijdens opvolgen en schrijven")
