...
class AdultAdSuiteRuntimeCockpitTests(TestCase):
    ...
    def test_adultadsuite_cockpit_renders_revenue_day_plan_for_logged_in_user(self):
        ...
        self.assertNotContains(response, "Open focusstand")
        self.assertNotContains(response, "?focus=1")
        self.assertNotContains(response, "Focusstand")
        self.assertNotContains(response, "focus=1")
        self.assertContains(response, "live operatorwerkvloer binnen AdultAdSuite")
        self.assertContains(response, "ga direct door naar het volgende.")
